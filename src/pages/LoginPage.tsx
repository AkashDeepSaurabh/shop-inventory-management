...
import { signInWithPopup, createUserWithEmailAndPassword, signInWithEmailAndPassword, fetchSignInMethodsForEmail } from 'firebase/auth';
...
const LoginPage = ({ setIsAuthenticated }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      // Check if email is registered in Firebase before proceeding with login
      const signInMethods = await fetchSignInMethodsForEmail(auth, email);

      // If no sign-in methods are found, the user is not registered, so display an error
      if (signInMethods.length === 0) {
        setError('No account found with this email. Please sign up first.');
        return;
      }

      // Proceed with login if the email is registered
